...
class CustomerAdapter(private var cars: LiveData<List<Car>>, private val listener: OnAdapterListener) : RecyclerView.Adapter<CustomerAdapter.CarViewHolder>() {
    inner class CarViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView)
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CarViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.items_customer, parent, false)
        return CarViewHolder(view)
    }
    override fun onBindViewHolder(holder: CarViewHolder, position: Int) {
        val car = cars.value?.get(position)
        holder.itemView.apply {
            findViewById<TextView>(R.id.tv_customer).text = cars.value?.get(position)?.customer ?:""
            findViewById<TextView>(R.id.tv_car).text = cars.value?.get(position)?.name ?:""
            val total = (cars.value?.get(position)?.price!! * cars.value?.get(position)?.day!!).toString()
